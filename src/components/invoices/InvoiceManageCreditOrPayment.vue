<template>
	<section class="main-content">
    	<div class="card">
			<div>
				<h1 class="text-2xl!" v-if="props.type === 'credits'">Manage Credits</h1>
				<h1 class="text-2xl!" v-if="props.type === 'payments'">Manage Payments</h1>
				...
				<div v-if="!data.loading">
					...
					<div class="mt-4">
						...
						<div class="overflow-auto! max-h-[280px] relative styled-scrollbar">
							<ApplyUnapplyTable :searching="data.searching" :headers="data.table.headers" :data="data.table.data" mode="add" @apply="(obj) => handleApply(obj, 'add')" :max="data.amount_left" :credit_left="data.amount_left" :type="props.type === 'credits' ? 'invoice-credit' : 'invoice-payment'"></ApplyUnapplyTable>
						</div>
					...
					<div v-if="data.applied.length > 0">
						...
						<div class="mt-4">
							<div class="overflow-auto! max-h-[280px] relative styled-scrollbar">
								<ApplyUnapplyTable :searching="false" :headers="data.table.headers" :data="data.applied" mode="edit" @apply="(obj) => handleApply(obj, 'edit')" @remove="removeApplied" @modify_amount_left="addToAmountLeft" @edit="handleEdit" :max="data.amount_left" :credit_left="data.amount_left" :type="props.type === 'credits' ? 'invoice-credit' : 'invoice-payment'"></ApplyUnapplyTable>
							</div>
						</div>
					</div>
...
const handleApply = (obj:TableRow, mode:string) : void => {
	...
	if(sum.lessThan(zero)){
		toastEvents.emit('toast', {
			type : 'error',
			message: `You can not apply ${to_be_sub.toFixed(2).toString()} because remaining ${amount_type} is ${left.toFixed(2).toString()}`
		});
		return ;
	}
...
const handleSubmit = async () : Promise<void> => {
	...
	if(sum.greaterThan(credit_amount)){
		toastEvents.emit('toast', {
			type : 'error',
			message: `You can not apply more than available ${amount_type} amount`
		});
		return ;
	}
	...
	try{
		data.disabled = true;
		let api_url = `manage-invoices/apply-unapply-credits/apply-unapply-credits`;
		if(props.type === 'payments'){
			api_url = `manage-invoices/apply-unapply-payments/apply-unapply-payments`;
		}
		const response = await api.patch(api_url, {
...
const fetchEntries = async () : Promise<void> => {
	...
	try{
		...
		let api_url = 'manage-invoices/apply-unapply-credits/search-credits';
		if(props.type === 'payments'){
			api_url = 'manage-invoices/apply-unapply-payments/search-payments';
		}
		...
		not_fully_applied_credits.forEach((row:TableRow) => {
			...
			to_be_assigned.push({
				id: row.id,
				[credit_or_payment] : row[credit_or_payment],
				total : row.total,
				left : row.left,
				amount : '',
				add: '',
...
const fetchAlreadyApplied = async () : Promise<void> => {
	...
	let api_url = `manage-invoices/apply-unapply-credits/fetch-already-applied`;

	if(props.type === 'payments'){
		api_url = `manage-invoices/apply-unapply-payments/fetch-already-applied`;
	}
...
onMounted(() : void => {
	...
	let headers = ['ID', 'Credit', 'Total', 'Left', 'Amount', '-'];
	if(props.type === 'payments'){
		headers[1] = 'Payment';
	}
