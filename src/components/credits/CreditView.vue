<template>
	<section class="main-content">
    	<div class="card">
			...
			<div>
				<h1 class="text-2xl!">View Credit</h1>
				...
				<div v-if="!data.loading">
					...
					<div class="lg:grid lg:grid-cols-12 lg:gap-4">
						...
						<div class="lg:col-span-4">
							<p>Applied amount : {{ data.credit.applied_amount }}</p>
						</div>
					</div>
					<div class="lg:grid lg:grid-cols-12 lg:gap-4 mt-[25px]">
						<div class="lg:col-span-4">
							<p>Amount left : {{ data.credit.amount_left_to_be_applied }}</p>
						</div>
						<div class="lg:col-span-4">
							<p>Client name : {{ data.credit.full_name }}</p>
						</div>
						<div class="lg:col-span-4">
							<p>Currency : {{ data.credit.credit_currency }}</p>
						</div>
					</div>
					<br>
					<br>
...
interface AppliedEntry {
	applied_amount_from_credits: string;
...
const fetchCredit = async (id : number) : Promise<void> => {
	data.loading = true;

	try{
		
		const response = await api.get(`manage-credits/view/${id}`);
