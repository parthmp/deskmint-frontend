					...
					<tr v-for="(row, ri) in local_table_data.rows" :key="ri">
						...
						<td v-for="(column2, ci2) in local_table_data.columns" :key="ci2">
							<span v-if="column2.label === 'actions'">
								<input-dropdown :options="row_actions" :row="row" @action="handleActionDropdown"></input-dropdown>
							</span>
							<span v-if="!Array.isArray(row[column2.label])">
								<span v-if="typeof row[column2.label] === 'object'">
									<span v-if="row[column2.label]?.type === 'label'">
										<span v-if="row[column2.label].highlight === 'success'" class="bg-green-500/18 pl-[12px]! pr-[12px]! pt-[3px]! pb-[3px]! rounded-2xl text-sm text-green-950 dark:text-green-200 whitespace-nowrap">{{ row[column2.label].text }}</span>
										<span v-if="row[column2.label].highlight === 'error'" class="bg-red-500/18 pl-[12px]! pr-[12px]! pt-[3px]! pb-[3px]! rounded-2xl text-sm text-red-950 dark:text-red-200 whitespace-nowrap">{{ row[column2.label].text }}</span>
										<span v-if="row[column2.label].highlight === 'info'" class="bg-blue-300/18 pl-[12px]! pr-[12px]! pt-[3px]! pb-[3px]! rounded-2xl text-sm text-blue-950 dark:text-blue-200 whitespace-nowrap">{{ row[column2.label].text }}</span>
									</span>
...
<script lang="ts">
import { IconTriangleInvertedFilled, IconTriangleInverted, IconTriangleFilled, IconEdit, IconTrash, IconTriangle, IconChevronLeft, IconChevronRight, IconEye } from '@tabler/icons-vue';
...
export default defineComponent({
	...
	methods : {
		...
		bootMeUp(omit = false) : void{
			...
			handleActionDropdown(obj : {action : string, row : { id : number}}){
				if(obj.action.toLowerCase() === 'view'){
					this.$router.push('/'+this.url_slug+'/view/'+obj.row.id);
				}else if(obj.action.toLowerCase() === 'edit'){
