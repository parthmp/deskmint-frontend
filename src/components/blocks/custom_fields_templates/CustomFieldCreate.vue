			...
			<form v-if="!data_loading" @submit.prevent="createClientCustomField" class="form">
				...
				<div class="lg:grid lg:grid-cols-12 gap-5">
					<div class="lg:col-span-4">
						<input-select :options="input_fields_options" @changed="changeEventFired" label="Input field" :required="true" prop_placeholder="Select" v-model="custom_field" ref="gen_custom_field"></input-select>
					</div>
					<div class="lg:col-span-4 mt-[20px] lg:mt-[0px]">
						<input-text label="Label" :required="true" prop_placeholder="Enter label text" v-model="label.value" :error="label.error" ref="gen_label"></input-text>
					</div>
					<div class="lg:col-span-4 mt-[20px] lg:mt-[0px]">
						<input-text label="Placeholder" prop_placeholder="Enter placeholder" :required="false" v-model="placeholder.value" :error="placeholder.error" ref="gen_placeholder"></input-text>
					</div>
				</div>
				
				<div class="lg:grid lg:grid-cols-12 gap-5">
					<div class="lg:col-span-4 mt-[20px]">
						<input-select :options="required_options" label="Is required?" :required="true" prop_placeholder="Select" v-model="required_flag" ref="gen_required_flag"></input-select>
					</div>
					<div class="lg:col-span-4 mt-[20px]">
				...
				<div class="mt-[20px]" v-show="show_options_textarea">
					<input-textarea label="Add options seperated by comma" :required="show_options_textarea_required" v-model="select_options.value" :error="select_options.error" ref="gen_select_options"></input-textarea>
				</div>
			...
			"label.value"(): void{
				if(this.label.value.trim() !== ''){
					...
					if(this.label.value.trim().length > 50){
						this.$refs.gen_label.showErrorsExplicitly();
