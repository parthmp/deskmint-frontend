	...
	<div>
		...
		<div v-if="!data.loading">
			...
			<form @submit.prevent="saveEmailSettingsContent">
				...
				<input-textarea v-model="data.email_content_invoice" :rows="6" :label="`Email content to send with invoice (${available_tags})`" :placeholder="`Email content to send with invoice`"></input-textarea>
				<input-textarea v-model="data.email_content_reminder" :rows="6" :label="`Email content to send for payment reminders (${available_tags})`" placeholder="Email content to send for payment reminders"></input-textarea>
...
<script lang="ts" setup>
	...
	const available_tags = 'Available tags are: {$client_first_name} , {$client_last_name} , {$invoice_date} , {$due_date} , {$invoice_number} , {$invoice_total} , {$unpaid_balance} , {$payment_url}';
