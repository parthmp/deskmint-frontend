<template>
	<div>
		...
		<div v-if="!data.loading">
			<p class="pl-1">Arrange how you want to display company details in the invoice</p>
...
<script lang="ts" setup>
	...
	const fetchCompanyDetailsFields = () : void => {
		data.loading = true;
		api.get('manage-invoice-settings-company-details').then(response => {
	...
	const saveClientDetailsSettings = () : void => {
		...
		api.post('manage-invoice-settings-company-details', {
			rows : data.rows	
		}).then(response => {
