<template>
	<section class="main-content">
    	<div class="card">
			<div>
				<h1 class="text-2xl!">Apply/Unapply Payment</h1>
 ...
 * type 3 : the entries that user applied and fetched from db, means already applied and deducted from the amount_left from payment amount, due does not include applied amount.
 */
...
const data = reactive<PaymentsApply>({
	payment_id : 0,
	amount : '100',
	amount_left: '100',
	currency_code : 'USD',
...
const handleApply = (obj:TableRow, mode:string) : void => {
	...
	if(sum.lessThan(zero)){
		toastEvents.emit('toast', {
			type : 'error',
			message: `You can not apply ${to_be_sub.toFixed(2).toString()} because remaining payment is ${left.toFixed(2).toString()}`
		});
		return ;
	}
...
const handleSubmit = async () : Promise<void> => {
	...
	const payment_amount = new Decimal(data.amount);
	...
	if(sum.greaterThan(payment_amount)){
		toastEvents.emit('toast', {
			type : 'error',
			message: `You can not apply more than available payment amount`
		});
		return ;
	}
	...
	try{
		data.disabled = true;
		const response = await api.patch('manage-payments/apply-unapply-payment', {
...
const fetchInvoices = async () : Promise<void> => {
	...
	try{
		...
		const response = await api.get('manage-payments/apply-unapply-search-invoices', {
			params : {
...
const fetchPayment = async () : Promise<void> => {
	try{
		const response = await api.get('manage-payments/apply-unapply-fetch-payment', {
			params : {
				payment_id : data.payment_id
			}
		});
...
const fetchAlreadyApplied = async () : Promise<void> => {
	const response = await api.get('manage-payments/fetch-already-applied', {
		params : {
			payment_id : data.payment_id
		}
	});
...
onMounted(() : void => {
	data.loading = true;
	let payment_id = +route.params.id;
	if(!isNaN(payment_id)){
