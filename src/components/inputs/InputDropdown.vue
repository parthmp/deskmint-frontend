<template>
	<div class="form-group" ref="dropdownWrapper">
		...
		<button id="dropdown_default" class="transition-all duration-300 flex gap-1 items-center bg-deskmint-cyan text-white py-[7px] px-[15px] rounded-lg w-full lg:w-[initial] cursor-pointer hover:hover:bg-deskmint-original-dark-plus" data-dropdown-toggle="dropdown" type="button" @click="show_menu = !show_menu"> {{ modelValue }} <span class="transition-all duration-300" :class="{'rotate-180':show_menu}"><IconChevronDown :size="22"></IconChevronDown></span></button>
		<transition name="fade-scale">
			<div v-if="show_menu" id="dropdown" class="z-10 absolute bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-[65%] lg:w-44 dark:bg-gray-700">
